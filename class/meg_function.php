...
function select_dealerautocomplate($STORED,$FLG,$EMPCODE,$CONDI1,$CONDI2,$CONDI3,$CONDI4,$CONDI5) {
    $conn = connect("RTMS");
    $data = "";
    $sql = "{call " . $STORED . "(?,?,?,?,?,?,?)}";
    $params = array(
        array($FLG, SQLSRV_PARAM_IN),
        array($EMPCODE, SQLSRV_PARAM_IN),
        array($CONDI1, SQLSRV_PARAM_IN),
        array($CONDI2, SQLSRV_PARAM_IN),
        array($CONDI3, SQLSRV_PARAM_IN),
        array($CONDI4, SQLSRV_PARAM_IN),
        array($CONDI5, SQLSRV_PARAM_IN)
    );
    $query = sqlsrv_query($conn, $sql, $params);
    while ($result = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC)) {
        $data .= "'" . $result['DEALER'] . "',";
...
function copyinsertoutsidegas($FLG, $CONDITION1) {
    $conn = connect("RTMS");
...
function copydeleteoutsidegas($FLG, $CONDITION1) {
    $conn = connect("RTMS");
    $sql = "{call megEditvehicletransportdocumentdriver_v2(?,?)}";
    $params = array(
        array($FLG, SQLSRV_PARAM_IN),
...
function editVehicletransportdocumentdriverconfrim_stc_cs($FLG, $CONDITION1) {
    $conn = connect("RTMS");
    $sql = "{call megEditvehicletransportdocumentdriverconfrim_stc_cs_v2(?,?)}";
    $params = array(
        array($FLG, SQLSRV_PARAM_IN),
        array($CONDITION1, SQLSRV_PARAM_IN)
    );
...
function editCopydiagramvehicletransportplan_newrccratc($TXT_FLG, $CONDITION1, $EMPLOYEENAME1, $EMPLOYEENAME2, $VEHICLEINFO, $CLUSTER, $JOBEND, $LOAD, $copydiagramdaterkupd, $copydiagramdatevlinupd, $copydiagramdatevloutupd, $copydiagramdatedealerinupd, $copydiagramdatereturnupd, $JOBSTART,$WORKTYPE) {
    $conn = connect("RTMS");
    $sql = "{call megEditvehicletransportplan_new_rccratc_v2(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
    $params = array(
        array($TXT_FLG, SQLSRV_PARAM_IN),
        array($CONDITION1, SQLSRV_PARAM_IN),
        array($EMPLOYEENAME1, SQLSRV_PARAM_IN),
...
function insCopydiagramvehicletransportplan_new_rccratc(
    ...
    //         . "?,?,?,?,?,"
    //         . "?,?,?,?,?,?,?,?,?,?,?,?,?)}";
    $sql = "{call megVehicletransportplancopydiagram_new_rccratc_v2(
        ?,?,?,?,?,
        ?,?,?,?,?,
        ?,?,?,?,?,
        ?,?,?,?,?,
...
function insVehicleinfo(
    ...
    $params = array(
        ...
        array($TOTALWEIGHT, SQLSRV_PARAM_IN),
        ...
        array($VEHICLEREGISTERFIRSTDATE, SQLSRV_PARAM_IN),
