...
if ($_GET['invoicecode'] == "" || $_GET['invoicecode'] == "") {
  $strExcelFileName = "RKLPJW_Billing.xls";
} else {
  $strExcelFileName = "RKLPJW_Billing" . $_GET['invoicecode'] . ".xls";
}
...
<body>
 ...
 <table style="width: 110%;">
      ...
      <tbody><br><br><br>
          <tr>
              <td colspan="11" style="font-size:22px">บริษัท ร่วมกิจรุ่งเรือง โลจิสติคส์ จำกัด (RUAMKIT RUNGRUENG LOGISTICS CO.,LTD.)</td>
        </tr>
        <tr>
              <td colspan="11" style="font-size:22px">สำนักงานใหญ่ เลขที่ 51 ม.4 ต.บ้านเก่า อ.พานทอง จ.ชลบุรี 20160 โทรศัพท์ : (038) 452824-5 โทรสาร : (038) 210396</td>
        ...
        <tr>
              <td colspan="11" style="font-size:22px" >เลขประจำตัวผู้เสียภาษี : 0105552003135</td>
        </tr>
        <tr>
              <td colspan="11">&nbsp;</td>
        </tr>
        ...
        <tr>
              <td colspan="5" style="text-align:left;font-size:22px">ลูกค้า บริษัท พีเจดีบเบิ้ลยู ออโต้อีวี จำกัด (มหาชน)</td>
              <td colspan="6" style="text-align:right;font-size:22px">เลขที่ <?= $result_seInvoicecode['INVOICECODE'] ?></td>
        </tr>
        <tr>
              <td colspan="5" style="text-align:left;font-size:22px">สาขาที่ 00002 ที่อยู่ 150/62 หมู่ที่ 9 ต.หนองขาม <br> อ.ศรีราชา  จ.ชลบุรี 20230</td>
              <td colspan="6" style="text-align:right;font-size:22px">วันที่ <?= $result_seduedate['DUEDATE'] ?></td>
        </tr>
        <tr>
              <td colspan="5" style="font-size:22px">เลขประจำตัวผู้เสียภาษี 0107565000603</td>
        </tr>
      </tbody>
  </table>
<br>
<table id="bg-table" width="100%" style="border-collapse: collapse;margin-top:8px;">
 <thead>
          ...
          <td   style="border-right:1px solid #000;padding:14px;text-align:center;font-size:12px"><b>หมายเลข PO</b></td>
          <td   style="border-right:1px solid #000;padding:14px;text-align:center;font-size:12px"><b>ทะเบียนรถ</b></td>
          <td   style="border-right:1px solid #000;padding:14px;text-align:center;font-size:12px"><b>ประเภทรถ</b></td>
...
<?php
  ...
  $condBilling1 = " AND c.COMPANYCODE = 'RKL' AND c.CUSTOMERCODE = 'PJW'";
  $condBilling2 = "";
  ...
  while ($result_seBilling = sqlsrv_fetch_array($query_seBilling, SQLSRV_FETCH_ASSOC)) {
      ...
      $holidaysplit = explode(",", $holiday);
        ...
        if (($result_seBilling['DATEVLIN_103'] == $holidaysplit[0]) || ($result_seBilling['DATEVLIN_103'] == $holidaysplit[1]) || ($result_seBilling['DATEVLIN_103'] == $holidaysplit[2]) || ($result_seBilling['DATEVLIN_103'] == $holidaysplit[3]) || ($result_seBilling['DATEVLIN_103'] == $holidaysplit[4])) {
          ...
          <td style="border-right:1px solid #000;padding:17px;text-align:center;font-size:12px"><?= $result_seBilling['DOCUMENTCODE'] ?></td>
          <td style="border-right:1px solid #000;padding:17px;text-align:center;font-size:12px"><?= $result_seBilling['THAINAME'] ?></td>
          <td style="border-right:1px solid #000;padding:17px;text-align:center;font-size:12px">เทรลเลอร์</td>
          ...
          <td style="border-right:1px solid #000;padding:17px;text-align:center;font-size:12px">1</td>
...
?>
      ...
      <tr style="border:1px solid #000;">
          ...
          <td  colspan="9" style="border-right:1px solid #000;padding:11px;text-align:center;font-size:12px"><b><?= convert($sumtotal1+$sumtotal2+$sumtotalholiday) ?></td>
