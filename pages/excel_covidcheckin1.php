...
$sql_getDep = "SELECT DEPARTMENTNAME FROM [dbo].[DEPARTMENT_NEW] WHERE 1=1 AND DEPARTMENTCODE = '" . $_GET['dep1'] . "'";
$params_getDep = array();
$query_getDep = sqlsrv_query($conn, $sql_getDep, $params_getDep);
$result_getDep = sqlsrv_fetch_array($query_getDep, SQLSRV_FETCH_ASSOC);

$sql_getSec = "SELECT [SECTIONCODE],[SECTIONNAME] FROM [dbo].[SECTION_NEW] WHERE [SECTIONCODE] = '" . $_GET['sec1'] . "'";
$params_getSec = array();
$query_getSec = sqlsrv_query($conn, $sql_getSec, $params_getSec);
$result_getSec = sqlsrv_fetch_array($query_getSec, SQLSRV_FETCH_ASSOC);

$strExcelFileName = "รายงานตัวโควิด-19 วันที่" . $_GET['datestart1'] . ".xls";
    ...
    <body>
        ...
        <table   style="border-collapse: collapse;margin-top:8px;font-size:10px" width="100%"  >
            <thead>
                <tr>
                    ...
                    <th colspan="44" style="border-top:1px solid #000;font-size:14px;border:1px solid #000;padding:6px;text-align:center">
                        <b>สรุปรายงานตัวโควิด-19 วันที่ <?= $_GET['datestart1'] ?> </b> (พื้นที่ : <?= $_GET['area1'] ?>, ฝ่าย : <?= $result_getDep['DEPARTMENTNAME'] ?>, แผนก : <?= $result_getSec['SECTIONNAME'] ?>)
            ...
            <tbody>
                <?php
                $i = 1;
                $dep1 = ($_GET['dep1'] == '') ? "" : "  AND b.[DEPARTMENTCODE]='" . $_GET['dep1'] . "'";
                $sec1 = ($_GET['sec1'] == '') ? "" : "  AND b.[SECTIONCODE]='" . $_GET['sec1'] . "'";
                $sql_seData1 = "SELECT DISTINCT a.[EMPLOYEECODE], 
CONVERT(NVARCHAR(10),a.CREATEDATE,103) AS 'CREATEDATE',c.FnameT+' '+LnameT AS 'nameFL'
...
WHERE 1=1
AND CONVERT(NVARCHAR(10),a.CREATEDATE,103) = '" . $_GET['datestart1'] . "'
AND b.AREA = '" . $_GET['area1'] . "'" . $dep1 . $sec1 . "
";
...
INNER JOIN [dbo].[EMPLOYEEEHR] c ON a.EMPLOYEECODE = c.PersonCode WHERE 1=1 
AND CONVERT(NVARCHAR(10),a.CREATEDATE,103) = '" . $_GET['datestart1'] . "'
AND b.AREA = '" . $_GET['area1'] . "'" . $dep1 . $sec1 . "))
";
