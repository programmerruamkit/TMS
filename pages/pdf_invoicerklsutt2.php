...
<tbody>
    <tr style="border:1px solid #000;font-size:24px">
      <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส  ช-001  
        <br>บริษัท โชนัน ยูนิเทค (ประเทศไทย) จำกัด 
        <br>สาขาที่ 00001 ที่อยู่ 242 นิคมอุตสาหกรรมเกตเวย์ซิตี้ หมู่ 7 ต.หัวสำโรง 
        <br>อ.แปลงยาว จ.ฉะเชิงเทรา 24190  		
        <br>โทร. 038-575725-9							
      </td>
...
while ($result_seBillinginvoice2 = sqlsrv_fetch_array($query_seBillinginvoice2, SQLSRV_FETCH_ASSOC)) {
  ...
  AND COMPANYCODE ='RKL' AND CUSTOMERCODE = 'SUTT' ";
  ...
  if ($holiday !='') {
    ...
    if ($holidaysplit[0] !='') {
      $sql_seCount0 = " SELECT COUNT(VEHICLETRANSPORTPLANID) AS 'COUNT_HOLIDAY0' 
        ...
        WHERE COMPANYCODE ='RKL' AND CUSTOMERCODE ='SUTT'
...
<tr style="border:1px solid #000;" >
    <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส  ช-001  
        ...
        <br>อ.แปลงยาว จ.ฉะเชิงเทรา 24190  		
        <br>โทร. 038-575725-9
        <br>เลขประจำตัวผู้เสียภาษี  0115544005922 							
    </td>
