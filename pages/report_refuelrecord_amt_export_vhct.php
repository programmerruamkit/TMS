...
if ($EXCELVHCT != "") { 
    ...
    // RKR-------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        // OPEN SECTION
            ...
            ->setCellValue('L3', 'ไมล์ต้น')
            ->setCellValue('M3', 'ไมล์ปลาย')
            ->setCellValue('N3', 'ระยะทาง')
            ->setCellValue('O3', 'ค่าเฉลี่ยที่ได้')
            ->setCellValue('P3', 'ต้นทาง')
            ->setCellValue('Q3', 'เส้นทาง')
            ->setCellValue('R3', 'รอบวิ่งงาน')
            ->setCellValue('S3', 'ประเภทงาน')
            ...
            $objPHPExcel->getActiveSheet()->getStyle('H:N')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT)->setVERTICAL(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            $objPHPExcel->getActiveSheet()->getStyle('O:S')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER)->setVERTICAL(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            ...
            while($objResult = sqlsrv_fetch_array($querystmm)) {  
                    ...
                    $SQL_CHK_OUTSIDE="SELECT TOP 1 VEHICLETRANSPORTPLANID,
                        ...
                        CASE WHEN (SELECT SUM(ISNULL(CAST(OSGS_AM AS DECIMAL(6,2)),0)) OSGS_AM FROM OUTSIDE_GAS_STATION WHERE OSGS_PLID = VEHICLETRANSPORTPLANID AND OSGS_TY = 4) > 0 THEN (SELECT SUM(ISNULL(CAST(OSGS_AM AS DECIMAL(6,2)),0)) OSGS_AM FROM OUTSIDE_GAS_STATION WHERE OSGS_PLID = VEHICLETRANSPORTPLANID AND OSGS_TY = 4)
                            ELSE 0 END OSGS_PM,
                        (SELECT SUM(ISNULL(CAST(OSGS_AM AS DECIMAL(6,2)),0)) OSGS_AM FROM OUTSIDE_GAS_STATION WHERE OSGS_PLID = VEHICLETRANSPORTPLANID) AS OSGS_AM,
                        OSGS_RM
                        FROM OUTSIDE_GAS_STATION
                        LEFT JOIN VEHICLETRANSPORTPLAN ON VEHICLETRANSPORTPLANID = OSGS_PLID
                        WHERE JOBNO = '$JNPLAN'
                        GROUP BY VEHICLETRANSPORTPLANID,OSGS_RM
                        ORDER BY OSGS_RM DESC";
                    ...
                    if(isset($RS_CHK_OUTSIDE["OSGS_AM"])){
                        if($RS_CHK_OUTSIDE["OSGS_AM"] > 0){
                            $CHKOUT=$RS_CHK_OUTSIDE["OSGS_OUT"];
                            $CHKPM=$RS_CHK_OUTSIDE["OSGS_PM"];
                            $CALOAM=($OAM+$CHKOUT)-$CHKPM;
                            $CALOAVG=number_format($DTE/$CALOAM,2);
                            $CHKRM=$RS_CHK_OUTSIDE["OSGS_RM"];                            
                            if(isset($CHKRM)){
                                $RSREMARK = ' ... '.$CHKRM;
                            }else{
                                $RSREMARK = '';
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('L' . $i, $MST);
                $objPHPExcel->getActiveSheet()->setCellValue('M' . $i, $MLE);
                $objPHPExcel->getActiveSheet()->setCellValue('N' . $i, $DTE);
                $objPHPExcel->getActiveSheet()->setCellValue('O' . $i, $CALOAVG);
                $objPHPExcel->getActiveSheet()->setCellValue('P' . $i, $JOBSTART);
                $objPHPExcel->getActiveSheet()->setCellValue('Q' . $i, $JOBEND);
                $objPHPExcel->getActiveSheet()->setCellValue('R' . $i, $ROUNDAMOUNT);
                $objPHPExcel->getActiveSheet()->setCellValue('S' . $i, $WORKTYPE);
            ...
            $objPHPExcel->getActiveSheet()->mergeCells('P'.$i.':Z'.$i);
            $objPHPExcel->getActiveSheet()->setCellValue('A' . $i, 'รวม');
            $objPHPExcel->getActiveSheet()->getStyle('A'.$i)->getFont()->setBold( true );                    
            $sheet->getStyle('H'.$i.':O'.$i)->getNumberFormat()->setFormatCode('0.00');
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('N' . $i, '=SUM(N5:N'.($i -1).')');
            $objPHPExcel->getActiveSheet()->setCellValue('O' . $i, '=N'.$i.'/K'.$i);            
    ...
    // CENTER-------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        // OPEN SECTION
            ...
            //     LEFT JOIN VEHICLETYPE VHCT ON VHCT.VEHICLETYPECODE = VHCIF.VEHICLETYPECODE                
            //     WHERE OTSN.REFUELINGDATE BETWEEN '$startymd' AND '$endymd'
            //     AND VHCTPP.VEHICLETRANSPORTPLANID IS NULL                
            //     ORDER BY CONVERT(VARCHAR (10),OTSN.REFUELINGDATE,20) ASC";
            $querystmm = sqlsrv_query($conn, $stmm );
            $i = 5;
            while($objResult = sqlsrv_fetch_array($querystmm)) {  
    ...
    // OUT-------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        // OPEN SECTION
            ...
            $stmm = "SELECT
                ...
                LEFT JOIN VEHICLEINFO VHCIF ON VHCIF.VEHICLEREGISNUMBER = OTSN.VEHICLEREGISNUMBER COLLATE Thai_CI_AI
                LEFT JOIN VEHICLETYPE VHCT ON VHCT.VEHICLETYPECODE = VHCIF.VEHICLETYPECODE
                LEFT JOIN VEHICLEINFO_CARPOOL VHCCP ON VHCCP.VEHICLEREGISNUMBER = OTSN.VEHICLEREGISNUMBER COLLATE Thai_CI_AI
                WHERE OTSN.REFUELINGDATE BETWEEN '$startymd' AND '$endymd'
                AND VHCTPP.VEHICLETRANSPORTPLANID IS NULL
                AND VHCCP.VEHICLEREGISNUMBER IS NULL
                AND (OTSN.JOBNO LIKE '%RKS%' OR OTSN.JOBNO LIKE '%RKR%' OR OTSN.JOBNO LIKE '%RKL%')
                ORDER BY CONVERT(VARCHAR (10),OTSN.REFUELINGDATE,20) ASC";
            $querystmm = sqlsrv_query($conn, $stmm );
            $i = 5;
            while($objResult = sqlsrv_fetch_array($querystmm)) {  
