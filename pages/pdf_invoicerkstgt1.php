...
$table_header3 = '<table style="width: 110%;">
    <thead>
        <tr>
            <td colspan="48" style="text-align:center;font-size:28px"><b>ใบส่งสินค้า</b></td>
        </tr>
    ...
    <tbody><br><br><br>
        <tr>
            <td colspan="48" style="width: 70%;font-size:22px">บริษัท ร่วมกิจรุ่งเรือง เซอร์วิส จำกัด (RUAMKIT RUNGRUENG SERVICES CO.,LTD.)</td>
       </tr>
       <tr>
       ...
       <tr>
            <td colspan="48" style="font-size:22px" >เลขประจำตัวผู้เสียภาษี : 0105544064899</td>
       </tr>
       <tr>
            <td colspan="48">&nbsp;</td>
       </tr>
       ...
       <tr>
            <td colspan="40" style="font-size:22px">ลูกค้า บริษัท โตโยด้า โกเซ (ประเทศไทย) จำกัด  (สำนักงานใหญ่)</td>
            <td colspan="20" style="text-align:right;font-size:22px">เลขที่ ' . $result_seInvoicecode['INVOICECODE'] . '</td>
       </tr>
       <tr>
            <td colspan="40" style="font-size:22px">700/489 ม.4  นิคมอุตสาหกรรมอมตะซิตี้ ต.บ้านเก่า  อ.พานทอง  จ.ชลบุรี 20160</td>
            <td colspan="20" style="text-align:right;font-size:22px">วันที่ 28/' . $date . ' เครดิต 30 วัน</td>
       </tr>
       <tr>
            <td colspan="40" style="font-size:22px">เลขประจำตัวผู้เสียภาษี 0105537013231</td>
            <td colspan="20" style="text-align:right;font-size:22px">วันที่ครบกำหนด ' . $result_seduedate['DUEDATE'] . '</td>
       </tr>
    </tbody>
</table>';
...
while ($result_seBilling = sqlsrv_fetch_array($query_seBilling, SQLSRV_FETCH_ASSOC)) {
    ...
    AND COMPANYCODE='".$_GET['companycode']."' AND CUSTOMERCODE = '".$_GET['customercode']."' AND JOBEND = '".$_GET['transportation']."'";
    ...
    $holidaysplit = explode(",", $holiday);
      ...
      if (($result_seBilling['DATEVLIN_103'] == $holidaysplit[0]) || ($result_seBilling['DATEVLIN_103'] == $holidaysplit[1]) || ($result_seBilling['DATEVLIN_103'] == $holidaysplit[2]) || ($result_seBilling['DATEVLIN_103'] == $holidaysplit[3]) || ($result_seBilling['DATEVLIN_103'] == $holidaysplit[4])) {
        ...
        <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 10%;font-size:48px">' . $i . '</td>
        ...
        <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 45%;font-size:48px">' . ($result_seBilling['JOBEND']  == 'INGY' ? 'INGY (Rayong)' : $result_seBilling['JOBEND']) . '</td>
      ...
      <tr style="border:1px solid #000;padding:17px;">
          <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 10%;font-size:48px"></td>
          <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 25%;font-size:48px">' . $result_seBilling['DATEVLIN_103'] . '</td>
          <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 30%;font-size:48px">' . $result_seBilling['DOCUMENTCODE'] . '</td>
          <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 25%;font-size:48px">' . $result_seBilling['THAINAME'] . '</td>
          <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 25%;font-size:48px">' . $result_seEmployeeehr['FnameT'] . '</td>
          <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 63%;font-size:48px">' . $result_seBilling['JOBSTART'] . '</td>
          <td style="border-right:1px solid #000;padding:17px;text-align:center;width: 45%;font-size:48px">Holiday</td>
          <td style="border-right:1px solid #000;padding:17px;text-align:right;width: 15%;font-size:48px">1.00</td>
