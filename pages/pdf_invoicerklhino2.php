...
$sql_sejobend = "SELECT TOP 1 TRANSPORTATION AS 'JOBEND',BILLINGDATE,PAYMENTDATE  FROM [dbo].[LOGINVOICE] WHERE INVOICECODE ='" . $result_seBillinginvoice['INVOICECODE'] . "'";
...
WHERE 1 = 1
AND a.COMPANYCODE ='RKL' AND a.CUSTOMERCODE ='HINO'
...
<tbody>
    <tr>
      <td colspan="1" rowspan="4" style="text-align:center"><img src="../images/logonew.png"></td>
        <td colspan="7" style="font-size:20px" ><b>บริษัท ร่วมกิจรุ่งเรือง โลจิสติคส์ จำกัด (RUAMKIT RUNGRUENG LOGISTICS CO., LTD.)<b></td>
   </tr>
   <tr>
...
<tbody>
    <tr style="border:1px solid #000;font-size:24px">
      <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส  ฮ-001-1
      
                <br>บริษัท ฮีโนมอเตอร์ส แมนูแฟคเจอริ่ง (ประเทศไทย) จำกัด
                <br>(สาขาที่ 00003)  ที่อยู่ 700/509  หมู่ที่ 2 ต.บ้านเก่า อ.พานทอง จ.ชลบุรี 20160		
                <br>เลขประจำตัวผู้เสียภาษี 0115546004940
...
while ($result_seBillinginvoice2 = sqlsrv_fetch_array($query_seBillinginvoice2, SQLSRV_FETCH_ASSOC)) {
  ...
  AND COMPANYCODE='RKL' AND CUSTOMERCODE = 'RKLHINO' ";
  ...
  $holidayprice = ($counts*1000);
    $tbody2 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;" valign="top">' . number_format($result_sumprice['SUMACTUALPRICE'], 2) . '</td>
...
$tfoot2 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td colspan="5" bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;text-align:center;font-size:22px;">' . convert(($result_sumprice['SUMACTUALPRICE'])) . '</td>
            <td style="border-right:1px solid #000;padding:4px;font-size:22px;">รวมทั้งสิ้น</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($result_sumprice['SUMACTUALPRICE']), 2) . '</td>
      </tr>
       ...
       <tr style="">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format((($result_sumprice['SUMACTUALPRICE']) * 1) / 100, 2) . '</td>

      </tr>
       <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;font-size:22px;">ยอดที่ต้องชำระ</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($result_sumprice['SUMACTUALPRICE']) - ((($result_sumprice['SUMACTUALPRICE']) * 1) / 100), 2) . '</td>
      </tr>
    </tfoot>';
...
<tbody>
    <tr style="border:1px solid #000;" >
        <td style="border-right:1px solid #000;padding:4px;width:70%">ลูกค้า : รหัส ฮ-001-1
        <br>บริษัท ฮีโนมอเตอร์ส แมนูแฟคเจอริ่ง (ประเทศไทย) จำกัด
        <br>(สาขาที่ 00003)  ที่อยู่ 700/509  หมู่ที่ 2 ต.บ้านเก่า 
...
while ($result_seBillinginvoice1 = sqlsrv_fetch_array($query_seBillinginvoice1, SQLSRV_FETCH_ASSOC)) {
    $tbody1 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;" valign="top">' . number_format($result_sumprice['SUMACTUALPRICE'], 2) . '</td>
      </tr>
';
...
$tfoot1 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:20px;">รวมเงิน</td>
            <td bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;font-size:20px;">' . convert($result_sumprice['SUMACTUALPRICE']) . '</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;">' . number_format($result_sumprice['SUMACTUALPRICE'], 2) . '</td>
      </tr>
