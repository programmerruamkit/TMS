...
WHERE 1 = 1
AND a.COMPANYCODE ='RKL' AND a.CUSTOMERCODE ='COPPERCORD'
...
<tbody>
    <tr style="border:1px solid #000;font-size:24px">
      <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส ค-014
...
while ($result_seBillinginvoice2 = sqlsrv_fetch_array($query_seBillinginvoice2, SQLSRV_FETCH_ASSOC)) {
  ...
  AND COMPANYCODE='RKL' AND CUSTOMERCODE = 'COPPERCORD' AND JOBEND = 'COPPERCORD'";
  ...
  if ($holiday !='') {
    ...
    if ($holidaysplit[0] !='') {
      $sql_seCount0 = " SELECT COUNT(VEHICLETRANSPORTPLANID) AS 'COUNT_HOLIDAY0' 
        ...
        WHERE COMPANYCODE ='RKL' AND CUSTOMERCODE ='COPPERCORD'
...
<tbody>
    ...
    <td style="border-right:1px solid #000;padding:4px;width:70%">ลูกค้า : รหัส ค-014
          <br>บริษัท คอปเปอร์ คอร์ด อินดัสทรี้ จำกัด
          <br>สำนักงานใหญ่  เลขที่ 700/657 ม.3 ต.บ้านเก่า
          <br>อ.พานทอง จ.ชลบุรี 20160
          <br>โทร.038-447048-9
