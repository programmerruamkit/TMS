...
$table_header3 = '<table style="width: 100%;">
    ...
    <tbody>
      <tr>
            <td colspan="2" style="font-size:22px">บริษัท ร่วมกิจรุ่งเรือง (1993) จำกัด (RUAMKIT RUNGRUENG (1993) CO., LTD.)</td>
       </tr>
       <tr>
            <td colspan="2" style="font-size:22px">สำนักงานใหญ่ เลขที่ 51 หมู่ 4 ตำบลบ้านเก่า อำเภอพานทอง จังหวัดชลบุรี 20160 </td>
       </tr>
       <tr>
            <td colspan="2" style="font-size:22px">โทรศัพท์ : 038-452824-5 โทรสาร : 038-210396</td>
       </tr>
       <tr>
            <td colspan="2">&nbsp;</td>
       </tr>
      ...
      <tr>
            <td style="width: 70%;font-size:22px">ลูกค้า บริษัท โตโยด้า โกเซ (ประเทศไทย) จำกัด  (สำนักงานใหญ่)</td>
            <td style="width: 30%;text-align:right;font-size:22px">เลขที่ ' . $result_seInvoicecode['INVOICECODE'] . '</td>
       </tr>
       <tr>
            <td style="width: 70%;font-size:22px">700/489 ม.4  นิคมอุตสาหกรรมอมตะซิตี้ ชลบุรี<br>ต.บ้านเก่า   อ.พานทอง  จ.ชลบุรี </td>
            <td style="width: 30%;text-align:right;font-size:22px">วันที่ ' .$datebilling . ' เครดิต 30 วัน</td>
       </tr>
       <tr>
            <td style="width: 70%;font-size:22px">เลขประจำตัวผู้เสียภาษี 0105537013231</td>
            <td style="width: 30%;text-align:right;font-size:22px">วันที่ครบกำหนด ' . $result_seInvoicecode['DULYDATE'] . '</td>
       </tr>
    </tbody>
</table>';
...
//     array($condBilling2, SQLSRV_PARAM_IN)
// );
$sql_seBilling = "SELECT CONVERT(VARCHAR(30), a.DATEVLIN, 103)  AS 'DATE_VLIN',
          a.THAINAME AS 'THAINAME',a.EMPLOYEENAME1 AS 'EMPLOYEENAME1',a.JOBSTART AS 'JOBSTART',a.JOBEND AS 'JOBEND',c.PRICE AS 'ACTUALPRICE',b.DOCUMENTCODE AS 'DOCUMENTCODE'
          ...
          WHERE 1 = 1
          AND a.COMPANYCODE ='RKR' AND a.CUSTOMERCODE ='TGT'
    ...
    if (($result_seBilling['DATE_VLIN'] == $holidaysplit[0]) || ($result_seBilling['DATE_VLIN'] == $holidaysplit[1]) || ($result_seBilling['DATE_VLIN'] == $holidaysplit[2]) || ($result_seBilling['DATE_VLIN'] == $holidaysplit[3]) || ($result_seBilling['DATE_VLIN'] == $holidaysplit[4])) {
      ...
      <tr style="border:1px solid #000;font-size:20px">
        ...
        <tr style="border:1px solid #000;">
          <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:20px"></td>
          <td style="border-right:1px solid #000;padding:4px;text-align:left;font-size:20px">' . $result_seBilling['DATE_VLIN'] . '</td>
          <td style="border-right:1px solid #000;padding:4px;text-align:left;font-size:20px">Holiday</td>
          <td style="border-right:1px solid #000;padding:4px;text-align:left;font-size:20px">' . $result_seBilling['THAINAME'] . '</td>
          <td style="border-right:1px solid #000;padding:4px;text-align:left;font-size:20px">' . $result_seEmployeeehr['FnameT'] . '</td>
          <td style="border-right:1px solid #000;padding:4px;text-align:left;font-size:20px">' . $result_seBilling['JOBSTART'] . '</td>
          <td style="border-right:1px solid #000;padding:4px;text-align:left;font-size:20px">' . $result_seBilling['JOBEND'] . '</td>
          <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px">1.00</td>
          <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px">1,000.00</td>
          <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px">1,000.00</td>
        </tr>
      ';
      $i++;
