...
WHERE 1 = 1
AND a.COMPANYCODE ='RKL' AND a.CUSTOMERCODE ='GMT'
...
<tbody>
    <tr style="border:1px solid #000;font-size:24px">
      <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส ก-004  
        <br>บริษัท กรีน เมทัลส์ (ประเทศไทย) จำกัด
        <br>สาขาที่ 00001 เลขที่ 219/18 หมู่ที่ 6 นิคมฯปิ่นทอง 3 
        <br>ต.บ่อวิน อ.ศรีราชา จ.ชลบุรี 20230  		
        <br>โทร. 038-575-557							
      </td>
...
while ($result_seBillinginvoice2 = sqlsrv_fetch_array($query_seBillinginvoice2, SQLSRV_FETCH_ASSOC)) {
  ...
  AND COMPANYCODE ='RKL' AND CUSTOMERCODE = 'GMT' ";
  ...
  if ($holiday !='') {
    ...
    if ($holidaysplit[0] !='') {
      $sql_seCount0 = " SELECT COUNT(VEHICLETRANSPORTPLANID) AS 'COUNT_HOLIDAY0' 
        ...
        WHERE COMPANYCODE ='RKL' AND CUSTOMERCODE ='GMT'
      ...
      $count1 = $result_seCount0['COUNT_HOLIDAY0'];
    }if ($holidaysplit[1] !='') {
      ...
      $count2 = $result_seCount1['COUNT_HOLIDAY1'];
    }if ($holidaysplit[2] !='') {
      ...
      $count3 = $result_seCount2['COUNT_HOLIDAY3'];
    }if ($holidaysplit[3] !='') {
  ...
  $counts = $counts+$count1+$count2+$count3+$count4;
  $holidayprice = ($counts*1000);

  
  
    $tbody2 .= '<tr style="border:1px solid #000;">
                    <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:22px;" height="300px" valign="top">' .  $i2. '</td>
...
<tbody>
    ...
    <tr>
        <td colspan="7" style="font-size:20px">สำนักงานสาขาที่ 1  เลขที่ 51 หมู่ 4 ตำบลบ้านเก่า อำเภอพานทอง จังหวัดชลบุรี 20160</td>
    </tr>
    <tr>
        <td colspan="7" style="font-size:20px">โทรศัพท์ : 038-452824-5 โทรสาร : 038-210396</td>
    </tr>
...
<tr style="border:1px solid #000;" >
    <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส  ก-004  
        ...
        <br>ต.บ่อวิน อ.ศรีราชา จ.ชลบุรี 20230  		
        <br>โทร. 038-575-557
        <br>เลขประจำตัวผู้เสียภาษี  0245548000991 							
    </td>
...
$thead1 = '<thead>
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:center;width:40%"><b>รายการ</b></td>
...
while ($result_seBillinginvoice1 = sqlsrv_fetch_array($query_seBillinginvoice1, SQLSRV_FETCH_ASSOC)) {
    $tbody1 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;" valign="top">' . number_format($result_sumprice['SUMACTUALPRICE']+$holidayprice, 2) . '</td>
      </tr>
';
...
$tfoot1 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:20px;">รวมเงิน</td>
            <td bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;font-size:20px;">' . convert($result_sumprice['SUMACTUALPRICE']+$holidayprice) . '</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;">' . number_format($result_sumprice['SUMACTUALPRICE']+$holidayprice, 2) . '</td>
      </tr>
