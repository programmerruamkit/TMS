...
//คำนวณเงินรวมของ 
$sql_sumprice = "SELECT SUM(CONVERT(INT,c.PRICE))AS 'SUMPRICE'  
FROM [dbo].[VEHICLETRANSPORTPLAN] a
INNER JOIN [dbo].[VEHICLETRANSPORTPRICE] c ON c.[VEHICLETRANSPORTPRICEID] = a.[VEHICLETRANSPORTPRICEID] 

WHERE 1 = 1
AND a.COMPANYCODE ='RKR'
AND a.CUSTOMERCODE ='TTAST'
AND (a.DOCUMENTCODE IS NOT NULL OR a.DOCUMENTCODE !='' )
AND a.STATUSNUMBER !='X'
AND a.VEHICLETYPE !='10W'
AND CONVERT(DATE,a.DATEWORKING) BETWEEN CONVERT(DATE,'".$result_seInvoice['BILLINGDATE']."',103) AND CONVERT(DATE,'".$result_seInvoice['PAYMENTDATE']."',103)";
...
<tbody>
    <tr style="border:1px solid #000;font-size:24px">
        <td style="border-right:1px solid #000;padding:4px;width:70%">ลูกค้า  :  รหัส ท-032
...
while ($result_seBillinginvoice2 = sqlsrv_fetch_array($query_seBillinginvoice2, SQLSRV_FETCH_ASSOC)) {
  ...
  $tbody2 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;" valign="top">' . number_format($result_sumprice['SUMPRICE'],2) . '</td>
...
$tfoot2 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td colspan="5" bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;text-align:center;font-size:22px;">' . convert(($result_sumprice['SUMPRICE'])) . '</td>
            <td style="border-right:1px solid #000;padding:4px;font-size:22px;">รวมทั้งสิ้น</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($result_sumprice['SUMPRICE']), 2) . '</td>
      </tr>
       ...
       <tr style="">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format((($result_sumprice['SUMPRICE']) * 1) / 100, 2) . '</td>

      </tr>
       <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;font-size:22px;">ยอดที่ต้องชำระ</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($result_sumprice['SUMPRICE']) - ((($result_sumprice['SUMPRICE']) * 1) / 100), 2) . '</td>
      </tr>
    </tfoot>';
...
while ($result_seBillinginvoice1 = sqlsrv_fetch_array($query_seBillinginvoice1, SQLSRV_FETCH_ASSOC)) {
    $tbody1 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;" valign="top">' . number_format($result_sumprice['SUMPRICE'], 2) . '</td>
      </tr>
';
...
$tfoot1 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:20px;">รวมเงิน</td>
            <td bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;font-size:20px;">' . convert($result_sumprice['SUMPRICE']) . '</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;">' . number_format($result_sumprice['SUMPRICE'], 2) . '</td>
      </tr>
