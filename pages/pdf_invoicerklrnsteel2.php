...
// $sql_sejobend = "SELECT TOP 1 TRANSPORTATION AS 'JOBEND'  FROM [dbo].[LOGINVOICE] 
// WHERE INVOICECODE ='" . $result_seBillinginvoice['INVOICECODE'] . "'";
...
<tbody>
    <tr>
      <td colspan="1" rowspan="4" style="text-align:center"><img src="../images/logonew.png"></td>
        <td colspan="7" style="font-size:24px" ><b>บริษัท ร่วมกิจรุ่งเรือง โลจิสติคส์ จำกัด (RUAMKIT RUNGRUENG LOGISTICS CO.,LTD.)<b></td>
   </tr>
   <tr>
    ...
    <tr>
      <td colspan="7" style="font-size:20px">เลขประจำตัวผู้เสียภาษี 0105552003135</td>
   </tr>
   <tr>
...
<tbody>
    <tr style="border:1px solid #000;font-size:24px">
      <td style="border-right:1px solid #000;padding:4px;width:75%;">ลูกค้า : รหัส อ-011
            <br>บริษัท อาร์ แอนด์ เอ็น สตีล จำกัด (สำนักงานใหญ่)
            <br>ที่อยู่ 2341/1 ม.6 ถ.เทพารักษ์ ต.เทพารักษ์ อ.เมืองสมุทรปราการ จ.สมุทรปราการ 10270
            <br>เลขประจำตัวผู้เสียภาษี 0115541004666
      </td>
        <td style="border-right:1px solid #000;padding:4px;text-align:center;width:30%">วันที่ : ' . $result_seBillinginvoice['DUEDATE'] . '</td>
   </tr>
...
$thead2 = '<thead>
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:center;width:28%;font-size:20px"><b>จำนวนเงิน</b></td>
...
while ($result_seBillinginvoice2 = sqlsrv_fetch_array($query_seBillinginvoice2, SQLSRV_FETCH_ASSOC)) {
  ...
  WHERE INVOICECODE ='".$result_seInvoice['INVOICECODE']."'
  AND COMPANYCODE='RKL' AND CUSTOMERCODE = 'RNSTEEL' AND JOBEND = 'RKLRNSTEEL'";
  ...
  $holidayprice = ($counts*1000);
    $tbody2 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;" valign="top">' . number_format($result_seBillinginvoice['SUMTOTAL']+$holidayprice, 2) . '</td>
...
';
    ...
    $i1++;
    $sumtotal = $sumtotal + $result_seBillinginvoice['SUMTOTAL'];
}
...
$tfoot2 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td colspan="5" bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;text-align:center;font-size:22px;">' . convert(($result_seBillinginvoice['SUMTOTAL']+$holidayprice)) . '</td>
            <td style="border-right:1px solid #000;padding:4px;font-size:22px;">รวมทั้งสิ้น</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($result_seBillinginvoice['SUMTOTAL']+$holidayprice), 2) . '</td>
      </tr>
       ...
       <tr style="">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format((($result_seBillinginvoice['SUMTOTAL']+$holidayprice) * 1) / 100, 2) . '</td>

      </tr>
       <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;font-size:22px;">ยอดที่ต้องชำระ</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($result_seBillinginvoice['SUMTOTAL']+$holidayprice) - ((($result_seBillinginvoice['SUMTOTAL']+$holidayprice) * 1) / 100), 2) . '</td>
      </tr>
    </tfoot>';
...
<tbody>
    <tr style="border:1px solid #000;" >
    <td style="border-right:1px solid #000;padding:4px;width:70%">ลูกค้า : รหัส อ-011
          <br>บริษัท อาร์ แอนด์ เอ็น สตีล จำกัด (สำนักงานใหญ่)
          <br>สำนักงานใหญ่ ที่อยู่ 2341/1 ม.6 ถ.เทพารักษ์ ต.เทพารักษ์ 
          <br>อ.เมืองสมุทรปราการ จ.สมุทรปราการ 10270
          <br>โทร. 02-385-1366-9,02-754-9380-1 แฟ็กซ์. 02-7557370-1
          <br>เลขประจำตัวผู้เสียภาษี 0115541004666
    </td>
...
while ($result_seBillinginvoice1 = sqlsrv_fetch_array($query_seBillinginvoice1, SQLSRV_FETCH_ASSOC)) {
    $tbody1 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;" valign="top">' . number_format($result_seBillinginvoice['SUMTOTAL']+$holidayprice, 2) . '</td>
      </tr>
';
...
$tfoot1 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:20px;">รวมเงิน</td>
            <td bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;font-size:20px;">' . convert($result_seBillinginvoice['SUMTOTAL']+$holidayprice) . '</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;">' . number_format($result_seBillinginvoice['SUMTOTAL']+$holidayprice, 2) . '</td>
      </tr>
...
<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:center;width:35%">ในนาม บริษัท ร่วมกิจรุ่งเรือง โลจิสติคส์ จำกัด<br><br><br><br><br><br>.....................................................................<br>ลงวันที่............./............/.............<br>ผู้มีอำนาจลงนาม / Authorized Signature</td>
    </tr>
    </tbody>
    <tfoot>
    </tfoot>
