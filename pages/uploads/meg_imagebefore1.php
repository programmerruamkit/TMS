...
// $destination_folder = "../pages/uploads/"; //upload directory ends with / (slash)
$destination_folder =  "C:/inetpub/wwwroot/RTMS/pages/uploads/"; //upload directory ends with / (slash)
...
if (isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if (!isset($_FILES['image_editbefore1']) || !is_uploaded_file($_FILES['image_editbefore1']['tmp_name'])) {
        die('Image file is Missing!'); // output error when above checks fail.
    }

    //uploaded file info we need to proceed
    $image_name = $_FILES['image_editbefore1']['name']; //file name
    $image_size = $_FILES['image_editbefore1']['size']; //file size
    $image_temp = $_FILES['image_editbefore1']['tmp_name']; //file temp
    ...
    if ($image_res) {
        ...
        $new_file_name =  'IMGBEFOR1_' . rand(0, 9999999999) . '.' . $image_extension;
        ...
        if (normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality)) {
            ...
            echo '<input type="text" style="display: none" id="IMGBEFORE1" value="'.$new_file_name.'">';
            echo '</div>';
