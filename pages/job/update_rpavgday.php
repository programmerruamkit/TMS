...
?>
<?php
    $sql_rpavgday = "SELECT * FROM vwRPAVGDAY";
    ...
    while($result_rpavgday = sqlsrv_fetch_array($query_rpavgday, SQLSRV_FETCH_ASSOC)) {
        ...
        $check1 = "SELECT * FROM TEMP_RPAVGDAY WHERE VEHICLETRANSPORTPLANID = '$VEHICLETRANSPORTPLANID' AND PersonCode = '$PersonCode' ";
        ...
        if($chk1null == ""){
            $sql = "INSERT INTO TEMP_RPAVGDAY (C3OLD,C3PLUS,C3MINUS,C3ZERO,PersonCode,EMPLOYEECODE1,EMPLOYEECODE2,CUSTOMERCODE,JOBEND,VEHICLETRANSPORTPLANID,DATEWORKING)
                SELECT * FROM vwRPAVGDAY A WHERE NOT A.VEHICLETRANSPORTPLANID IN (SELECT DISTINCT B.VEHICLETRANSPORTPLANID FROM TEMP_RPAVGDAY B WHERE B.VEHICLETRANSPORTPLANID = A.VEHICLETRANSPORTPLANID)";
            $stmt = sqlsrv_query($conn, $sql);
        }else{
            $sql = "UPDATE TEMP_RPAVGDAY SET C3OLD=?,C3PLUS=?,C3MINUS=?,C3ZERO=? WHERE RPAVGDAY_ID=?";
