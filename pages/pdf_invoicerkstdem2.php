...
$table_header21 = '<br>';
if ($result_ChkJobend['JOBEND'] == 'TDEM/SR(RKS)') {
    $table_header2 = '<table width="100%" >
    <tbody>
    <tr>
        <td colspan="1" rowspan="4" style="text-align:center"><img src="../images/logonew.png"></td>
        <td colspan="7" style="font-size:24px" ><b>บริษัท ร่วมกิจรุ่งเรือง เซอร์วิส จำกัด (RUAMKIT RUNGRUENG SERVICES CO.,LTD.) CO.,LTD.)<b></td>
    </tr>
    <tr>
    ...
    <tbody>
        <tr style="border:1px solid #000;font-size:24px">
            <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส ต-002
                <br>บริษัท โตโยต้า มอเตอร์ ประเทศไทย จำกัด
                <br>(สาขาที่ 00010) ที่อยู่ 186/2 ม.1 ถ.ทางรถไฟเก่า ต.สำโรงใต้ 
                <br>อ.พระประแดง จ.สมุทรปราการ 10130
                <br>โทร. 02-386-2106
                <br>เลขที่ประจำตัวผู้เสียภาษี 0105505001679
            </td>
            <td style="padding:4px;width:20%">เลขที่ :
                <br>เลขที่ใบวางบิล :
            </td>
            <td style="border-right:1px solid #000;padding:4px;font-size:22px;width:20%">' . $result_seBillinginvoice['BILLINGDATE'] . '
                <br> ' . $result_seBillinginvoice['INVOICECODE'] . '
            </td>
        </tr>
    </tbody>
    </table>';
    ...
    <tbody>
        <tr style="border:1px solid #000;font-size:24px">
            <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส ต-002
                <br>บริษัท โตโยต้า มอเตอร์ ประเทศไทย จำกัด
                <br>(สาขาที่ 00019) ที่อยู่ 99 หมู่.2 ต.ตลาดขวาง 
                <br>อ.บ้านโพธิ์ จ.ฉะเชิงเทรา 24140
                <br>โทร. 02-790-5998
                <br>เลขที่ประจำตัวผู้เสียภาษี 0105505001679
            </td>
            <td style="padding:4px;width:20%">เลขที่ :
...
while ($result_seBillinginvoice2 = sqlsrv_fetch_array($query_seBillinginvoice2, SQLSRV_FETCH_ASSOC)) {
    ...
    $sql_ChkJobend = "SELECT COMPANYCODE,CUSTOMERCODE,INVOICECODE,JOBEND,HOLIDAY,BILLINGPRPO
        ...
        WHERE INVOICECODE ='".$result_seBillinginvoice2['INVOICECODE']."'
        AND COMPANYCODE='RKS' AND CUSTOMERCODE = 'TDEM' ";
    ...
    $sql_sumprice = "SELECT SUM(CONVERT(DECIMAL(10,3),a.ACTUALPRICE)) AS 'SUMACTUALPRICE'
        ...
        a.THAINAME AS 'THAINAME',a.EMPLOYEENAME1 AS 'EMPLOYEENAME1',a.JOBSTART AS 'JOBSTART',a.JOBEND AS 'JOBEND',
        c.PRICE AS 'ACTUALPRICE1',[STATUS],STATUSNUMBER,(CASE
        WHEN a.STATUSNUMBER = 'X' THEN c.PRICE/2
        ELSE c.PRICE
        END)  AS 'ACTUALPRICE'
        ...
        WHERE 1 = 1
        AND a.COMPANYCODE ='RKS' AND a.CUSTOMERCODE ='TDEM'
    ...
    $sql_seHoliday = " SELECT HOLIDAY AS 'DATE' 
        FROM [dbo].[BILLING_HOLIDAY] 
        WHERE INVOICECODE ='".$result_seInvoice['INVOICECODE']."'
        AND COMPANYCODE='RKS' AND CUSTOMERCODE = 'TDEM' AND JOBEND = '".$result_ChkJobend['JOBEND']."'";
  ...
  if ($holiday !='') {
    ...
    if ($holidaysplit[0] !='') {
      $sql_seCount0 = " SELECT COUNT(VEHICLETRANSPORTPLANID) AS 'COUNT_HOLIDAY0' 
        ...
        WHERE COMPANYCODE ='RKS' AND CUSTOMERCODE ='TDEM'
   ...
   $holidayprice = ($counts*1000);
    ...
    $tbody2 .= '<tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:22px;" height="60px" valign="top">' . $i1 . '</td>
...
';
    ...
    $i1++;
    $sumtotal = $sumtotal + $result_sumprice['SUMACTUALPRICE'];
}
...
$tfoot2 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td colspan="5" bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;text-align:center;font-size:22px;">' . convert(($sumtotal+$holidayprice)) . '</td>
            <td style="border-right:1px solid #000;padding:4px;font-size:22px;">รวมทั้งสิ้น</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($sumtotal+$holidayprice), 2) . '</td>
      </tr>
       ...
       <tr style="">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format((($sumtotal+$holidayprice) * 1) / 100, 2) . '</td>

      </tr>
       <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;font-size:22px;">ยอดที่ต้องชำระ</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($sumtotal+$holidayprice) - ((($sumtotal+$holidayprice) * 1) / 100), 2) . '</td>
      </tr>
    </tfoot>';
...
if ($result_ChkJobend['JOBEND'] == 'TDEM/SR(RKS)') {
    ...
    <tbody>
        <tr style="border:1px solid #000;" >
        <td style="border-right:1px solid #000;padding:4px;width:70%">ลูกค้า : รหัส ต-006
            <br>บริษัท โตโยต้า มอเตอร์ ประเทศไทย จำกัด 
            <br>(สาขาที่ 00010) ที่อยู่ 186/2 ม.1 ถ.ทางรถไฟเก่า ต.สำโรงใต้
            <br>อ.พระประแดง จ.สมุทรปราการ 10130
            <br>โทร. 02-3862106
            ...
            <br> ' . $result_seBillinginvoice['BILLINGDATE'] . '
            <br> 30 วัน
    ...
    <tbody>
        <tr style="border:1px solid #000;" >
        <td style="border-right:1px solid #000;padding:4px;width:70%">ลูกค้า : รหัส ต-005
            <br>บริษัท โตโยต้า มอเตอร์ ประเทศไทย จำกัด 
            <br>(สาขาที่ 00019) ที่อยู่  99 หมู่ 2 ต.ลาดขวาง อ.บ้านโพธิ์ จ.ฉะเชิงเทรา 24140
            <br>โทร. 02-790-5998
            <br>เลขประจำตัวผู้เสียภาษี 0105505001679
        </td>
            <td style="padding:4px;width:20%">เลขที่ :
...
$condBillinginvoice21 = " AND a.INVOICECODE = '" . $result_seBillinginvoice['INVOICECODE'] . "'";
...
while ($result_seBillinginvoice1 = sqlsrv_fetch_array($query_seBillinginvoice1, SQLSRV_FETCH_ASSOC)) {
    $tbody1 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;" valign="top">' . number_format($sumtotal+$holidayprice, 2) . '</td>
      </tr>
';
...
$tfoot1 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:20px;">รวมเงิน</td>
            <td bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;font-size:20px;">' . convert($sumtotal+$holidayprice) . '</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;">' . number_format($sumtotal+$holidayprice, 2) . '</td>
      </tr>
