...
$sql_sesumtotal = "SELECT SUM(CONVERT(INT,ACTUALPRICE)) AS 'SUMTOTAL' FROM VEHICLETRANSPORTPLAN WHERE COMPANYCODE ='RKL' AND CUSTOMERCODE ='TTTC'
AND JOBEND ='".$result_sejobend['JOBEND']."'
AND CONVERT(DATE,DATEVLIN) BETWEEN CONVERT(DATE,'".$result_seInvoice['BILLINGDATE']."',103) AND CONVERT(DATE,'".$result_seInvoice['PAYMENTDATE']."',103) 
AND DOCUMENTCODE IS NOT NULL AND  DOCUMENTCODE != ''";
$params_sesumtotal = array();
$query_sesumtotal = sqlsrv_query($conn, $sql_sesumtotal, $params_sesumtotal);
$result_sesumtotal = sqlsrv_fetch_array($query_sesumtotal, SQLSRV_FETCH_ASSOC);
...
<tbody>
    <tr style="border:1px solid #000;font-size:24px">
      <td style="border-right:1px solid #000;padding:4px;width:70%;">ลูกค้า : รหัส ต-001
            <br>บริษัท โตโยต้า ทูโซ (ประเทศไทย) จำกัด
...
while ($result_seBillinginvoice2 = sqlsrv_fetch_array($query_seBillinginvoice2, SQLSRV_FETCH_ASSOC)) {
  ...
  AND COMPANYCODE='".$result_seInvoice['COMPANYCODE']."' AND CUSTOMERCODE = '".$result_seInvoice['CUSTOMERCODE']."' AND JOBEND = '".$result_sejobend['JOBEND']."'";
  ...
  $holidayprice = ($counts*1000);
    $tbody2 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;" valign="top">' . number_format($result_sesumtotal['SUMTOTAL']+$holidayprice, 2) . '</td>
...
';
    ...
    $i1++;
    $sumtotal = $sumtotal + $result_sesumtotal['SUMTOTAL'];
}
...
$tfoot2 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td colspan="5" bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;text-align:center;font-size:22px;">' . convert(($result_sesumtotal['SUMTOTAL']+$holidayprice)) . '</td>
            <td style="border-right:1px solid #000;padding:4px;font-size:22px;">รวมทั้งสิ้น</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($result_sesumtotal['SUMTOTAL']+$holidayprice), 2) . '</td>
      </tr>
       ...
       <tr style="">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format((($result_sesumtotal['SUMTOTAL']+$holidayprice) * 1) / 100, 2) . '</td>

      </tr>
       <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;font-size:22px;">ยอดที่ต้องชำระ</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:22px;">' . number_format(($result_sesumtotal['SUMTOTAL']+$holidayprice) - ((($$result_sesumtotal['SUMTOTAL']+$holidayprice) * 1) / 100), 2) . '</td>
      </tr>
    </tfoot>';
...
<tbody>
   ...
   <tr>
    ...
    <tr>
      <td colspan="7" style="font-size:20px">เลขประจำตัวผู้เสียภาษี 010555203135</td>
   </tr>
...
<tbody>
    <tr style="border:1px solid #000;" >
    <td style="border-right:1px solid #000;padding:4px;width:70%">ลูกค้า : รหัส ต-001
          <br>บริษัท โตโยด้า ทูโช (ประเทศไทย) จำกัด
          <br>สำนักงานใหญ่ 607 ถ.อโศก-ดินแดง
          <br>แขวงดินแดง เขตดินแดง กรุงเทพฯ 10400
          <br>โทร. 02-6255555
          <br>เลขประจำตัวผู้เสียภาษี 0105500000011
    </td>
...
while ($result_seBillinginvoice1 = sqlsrv_fetch_array($query_seBillinginvoice1, SQLSRV_FETCH_ASSOC)) {
    $tbody1 .= '<tr style="border:1px solid #000;">
        ...
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;" valign="top">' . number_format($result_sesumtotal['SUMTOTAL']+$holidayprice, 2) . '</td>
      </tr>
';
...
$tfoot1 = '</tbody>
    ...
    <tr style="border:1px solid #000;">
        <td style="border-right:1px solid #000;padding:4px;text-align:center;font-size:20px;">รวมเงิน</td>
            <td bgcolor="#CCCCCC" style="border-right:1px solid #000;padding:4px;font-size:20px;">' . convert($result_sesumtotal['SUMTOTAL']+$holidayprice) . '</td>
        <td style="border-right:1px solid #000;padding:4px;text-align:right;font-size:20px;">' . number_format($result_sesumtotal['SUMTOTAL']+$holidayprice, 2) . '</td>
      </tr>
