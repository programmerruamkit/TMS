...
$strExcelFileName = "รายงานน้ำมัน STM-TMT SR_TAW วันที่ ".$_GET['datestart'] .".xls";
  ...
  <thead>
        ...
        while ($result_seBillings = sqlsrv_fetch_array($query_seBillings, SQLSRV_FETCH_ASSOC)){
          ...
          $sql_seTrip = "SELECT MAX(a.TRIP) AS 'TRIP' FROM(
                      ...
                      WHERE COMPANYCODE  ='" . $_GET['companycode'] ."' AND CUSTOMERCODE IN ('TMT','TAW')
          ...
          $sql_seOilavg= "SELECT DISTINCT OILAVERAGE AS 'OILAVG' FROM [dbo].[OILAVERAGE]
                      WHERE ACTIVESTATUS ='1'
                      AND COMPANYCODE ='" .$_GET['companycode'] . "'
                      AND CUSTOMERCODE IN('TMT','TAW')
          ...
          ///////////////////////////////งานนอก/////////////////////////////////////////
          $sql_seOilavg1 = "SELECT DISTINCT OILAVERAGE AS 'OILAVG' FROM [dbo].[OILAVERAGE]
                      WHERE ACTIVESTATUS ='1'
          ...
          if ($result_seData['TRIP'] == '') {
            ...
            if ($result_seData['JOBSTART'] == 'STM->TMT SR') {
              $Deliver = 'STM';
            }else {
              $Deliver = 'TAW';
            }
      ...
      <tr style="border:1px solid #000;" >
        ...
        <!-- TOTAL -->
        <td colspan="1" style="border-right:1px solid #000;padding:3px;text-align:center;"><b><?=number_format($result_seData['OIL']+$result_seData1['OIL'],2)?></b></td>
        <td colspan="1" style="border-right:1px solid #000;padding:3px;text-align:center;"><b><?=number_format($MoneyOil)+number_format($MoneyOil1)?></b></td>
      </tr>
      <?php
      $i++;
      }
